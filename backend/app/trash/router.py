...
를 노출한다. 세 경로 모두 editor 이상으로 게이팅되며 판정은 s01 resolver(`require_ws_role`)
...
- 세 경로 모두 EDITOR 최소 role. viewer/비멤버→403(INV-2), admin→bypass(INV-3).
...
_PURGE_DESCRIPTION = (
    ...
    "백엔드는 요청 본문 없이 이 비가역성만 표기한다. editor 이상만 허용된다."
)
...
def list_trash(
    ...
    _ctx: AuthContext = Depends(require_ws_role(Role.EDITOR)),
    service: TrashService = Depends(get_trash_service),
) -> Page[TrashBundleRead]:
    """워크스페이스 휴지통의 trashed 묶음 목록을 페이지로 조회한다 (Req 1.1·1.7, editor 이상).

    s05 `{id}`→workspace_id 브리지(`require_ws_role(EDITOR)`)로 게이트를 강제한다(viewer/
...
def restore_bundle(
    bundleId: int,
    db: Session = Depends(get_db),
    _ctx: AuthContext = Depends(ws_role_for_bundle(Role.EDITOR)),
    service: TrashService = Depends(get_trash_service),
) -> None:
    """휴지통 묶음을 복구해 묶음 전체를 active 로 되돌린다 (Req 2.1·2.5, editor 이상).

    s10 묶음→WS 어댑터(`ws_role_for_bundle(EDITOR)`)로 묶음 루트 문서 id → workspace_id 를
    매핑해 게이트를 강제한다(묶음 문서 부재→404, viewer/비멤버→403, admin→bypass 판정은 s01
...
def purge_bundle(
    ...
    """휴지통 묶음을 즉시 완전삭제해 묶음 전체를 deleted(종착)로 전환한다 (Req 3.1·3.6, editor 이상).

    s10 묶음→WS 어댑터(`ws_role_for_bundle(EDITOR)`)로 게이트를 강제한다(묶음 문서 부재→404,
    viewer/비멤버→403, admin→bypass 판정은 s01 소유). 물리 삭제 없이 상태 전환만 수행하며

...
표 셀 정렬(`|:--|:-:|--:|`)도 같은 축의 divergence 였다 — markdown-it 은 정렬을 인라인
`style="text-align:…"` 로 내보내는데 `style` 이 allowlist 에 없어 정렬이 통째로 소실됐다.
`style` 을 여는 대신 core 규칙으로 표현 속성 `align` 으로 정규화해(에디터 Toast 산출과 동일 형태)
새니타이즈를 통과시킨다.
...
# GFM 표 정렬(`|:---|:--:|---:|`)이 허용하는 값. 그 외 값은 정렬 속성을 붙이지 않는다.
...
# 표 셀 정렬(`align="left|center|right"`)을 보존한다. markdown-it 은 정렬을 인라인
# `style="text-align:…"` 로 내보내는데 nh3 기본 allowlist 에 `style` 이 없어 통째로 제거되고,
# 그 결과 게스트 공개 뷰에서 정렬이 소실됐다. `style` 을 여는 대신(임의 CSS 주입 표면) 아래
# `_table_align_attr_rule` 이 정렬을 무력한 표현 속성 `align` 으로 정규화하고 그 속성만 허용한다.
# 이는 에디터(Toast UI)가 내보내는 형태와도 동일해 두 렌더 경로의 DOM 이 수렴한다
# (실제 정렬 적용은 prose.css 의 `th[align=…]`/`td[align=…]` 규칙이 담당).
#
# SECURITY: `align` 은 URL·스크립트를 담을 수 없는 표현 속성이며, 허용 값 밖의 문자열은 CSS
# 규칙에 매칭되지 않아 아무 효과도 없다(사용자가 raw `<td align=…>` 을 주입해도 무력).
_ALLOWED_ATTRIBUTES.setdefault("th", set()).add("align")
_ALLOWED_ATTRIBUTES.setdefault("td", set()).add("align")


def _table_align_attr_rule(state: StateCore) -> None:
    """표 셀 토큰의 `style="text-align:X"` 를 `align="X"` 로 정규화하는 core 규칙.

    markdown-it 의 표 규칙은 정렬을 인라인 style 로 표현하지만, 새니타이즈 allowlist 는
    `style` 을 허용하지 않는다(임의 CSS 주입 방지). 정렬 정보를 잃지 않도록 렌더 직전에
    표현 속성 `align` 으로 옮긴다 — 에디터 Toast Viewer 의 산출 DOM 과 동일한 형태다.
    """
    for token in state.tokens:
        ...
        if prop.strip() != "text-align":
            continue
        token.attrs.pop("style", None)
        alignment = value.strip()
        if alignment in _TABLE_ALIGNMENTS:
...
class MarkdownRenderer:
    ...
    def __init__(self) -> None:
        ...
        # 표 셀 정렬을 새니타이즈에서 살아남는 `align` 속성으로 정규화(위 규칙 주석 참조).
        self._md.core.ruler.push("table_align_attr", _table_align_attr_rule)

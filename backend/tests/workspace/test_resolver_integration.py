...
(Req 4.2·4.3·4.4·4.5·4.6·4.7, INV-1·2·3). 아울러 생성 owner화(1.1)·상세 읽기 전역
개방(3.5·3.8, 멤버·비멤버 모두 200)·멤버 오류(3.2·3.3)·삭제 캐스케이드(2.5) 왕복을 함께
단언한다.
...
def test_owner_gate_lets_owner_and_admin_only(ws_harness):
    ...
    # member 는 OWNER 미달 → 403 (INV-1·2, Req 5.4).
    r_member = patch(member)
    assert r_member.status_code == 403, f"member 는 403 이어야 한다: {r_member.text}"
...
# --- 3. 상세 읽기 전역 개방 (Req 3.5·3.8) ------------------------------------


def test_member_and_non_member_can_read_detail(ws_harness):
    """상세 읽기는 전역 개방 — member 는 물론 비멤버 활성 사용자도 200 (Req 3.5·3.8, 403 제거)."""
    admin = ws_harness.login_admin()
    member_id = ws_harness.create_user(admin, "member-3", name="멤버")
    ws_harness.create_user(admin, "nonmember-3", name="비멤버")
    ws_harness.create_user(admin, "owner-3", name="오너")

    owner = ws_harness.login("owner-3", "ws-member-pass-123")
    member = ws_harness.login("member-3", "ws-member-pass-123")
    nonmember = ws_harness.login("nonmember-3", "ws-member-pass-123")

    ws_id = _create_workspace(owner, "상세 읽기 공간")
    assert _add_member(owner, ws_id, member_id, "member").status_code == 201

    # member → 200, role 은 호출자 관점(member).
    r_member = member.get(f"/workspaces/{ws_id}")
    assert r_member.status_code == 200, f"member 상세는 200: {r_member.text}"
    assert r_member.json()["id"] == ws_id
    assert r_member.json()["role"] == "member"
    # 비멤버 활성 사용자도 200(읽기 개방), role 은 None.
    r_non = nonmember.get(f"/workspaces/{ws_id}")
    assert r_non.status_code == 200, f"비멤버 상세도 200(읽기 개방): {r_non.text}"
    assert r_non.json()["id"] == ws_id
    assert r_non.json()["role"] is None
...
def test_member_add_conflict_and_not_found(ws_harness):
    ...
    admin = ws_harness.login_admin()
    member_id = ws_harness.create_user(admin, "member-4", name="멤버")
...
def test_delete_removes_workspace_and_all_memberships(ws_harness):
    ...
    admin = ws_harness.login_admin()
    member_id = ws_harness.create_user(admin, "member-5", name="멤버")
    ...
    # 삭제 전: 워크스페이스·2개 멤버십(owner 자동 + member)이 존재.
    assert _member_row(ws_harness, ws_id, owner_id) is not None
    assert _member_row(ws_harness, ws_id, member_id) is not None

...
- list_workspaces: admin·비-admin 모두 list_all(전체) 단일 경로 — 목록 읽기 전역 개방.
  각 항목 role 은 호출자 자신의 멤버십(비멤버는 None)이며 Page[WorkspaceRead] 를 반환한다
  (Req 1.3·1.4).
...
class _FakeWorkspaceRepo:
    ...
    def __init__(
        ...
        *,
        ...
        # list_all 은 (Workspace, role) 튜플 목록을 반환하며 role 은 str|None 이다
        # (호출자 자신의 멤버십, 비멤버 WS 는 None).
...
def test_list_workspaces_non_admin_also_uses_list_all():
    """비-admin 도 멤버 스코프가 아니라 전체 목록 경로(`list_all`)를 탄다(목록 읽기 전역 개방)."""
    db = _FakeDb()
    ws_repo = _FakeWorkspaceRepo()
    ws_repo.list_all_result = ([(_make_ws(ws_id=3), "member")], 1)
    service = WorkspaceService(ws_repo, _FakeMembershipRepo(db))

    page = service.list_workspaces(db, USER_CTX, limit=20, offset=5)

    assert ws_repo.list_all_calls == [(USER_CTX.user_id, 20, 5)]
    assert page.total == 1
    assert [i.id for i in page.items] == [3]
    # 각 항목 role 은 호출자 멤버십 role 을 반영한다(Req 1.1).
    assert page.items[0].role == MemberRole.MEMBER


def test_list_workspaces_non_admin_sees_non_member_workspace_with_null_role():
    """비-admin 비멤버 워크스페이스도 목록에 포함되며 그 항목의 role 은 None 이다.

    가시성(전역)과 권한(멤버십 role)의 분리를 관찰한다: 항목은 보이되 role 이 null 이므로
    상위 계층이 읽기 전용으로 렌더링·게이팅할 수 있다.
    """
    db = _FakeDb()
    ws_repo = _FakeWorkspaceRepo()
    ws_repo.list_all_result = (
...
def test_list_workspaces_preserves_existing_fields_with_role_added():
    ...
    ws_repo.list_all_result = ([(ws, "owner")], 1)
    service = WorkspaceService(ws_repo, _FakeMembershipRepo(db))

...
``{workspace_id}``) 어댑터의 이름 브리징이 실제로 일어나 owner→200 / editor·viewer·
비멤버→403 / admin→200 으로 s01 semantics 를 그대로 재현하고, s01 공통 ``require_admin`` 을
부착한 라우트가 admin→200 / 비-admin→403 으로 게이팅됨을 확인한다.

격리: ``test_integration_wiring.py`` 와 동일한 확립된 패턴을 재사용한다. ``DB_NAME`` 을
전용 테스트 DB(``notion_lite_test``)로 바꾸고 ``get_settings`` 캐시를 비운 뒤 그 시점 URL 로
...
class _Wiring:
    ...
    def __init__(
        self, app, workspace_id, owner_id, editor_id, viewer_id, non_member_id, admin_id
    ):
        self.app = app
...
def wiring():
    ...
    try:
        ...
        editor = _make_user(seed, login_id="adapter-editor")
        viewer = _make_user(seed, login_id="adapter-viewer")
...
def test_adapter_editor_denied_for_owner_route_403(wiring):
    """editor 는 OWNER 요구 라우트에서 위계 미달 → 403 (Req 4.3, INV-2)."""
    with TestClient(wiring.app) as client:
...
def test_adapter_viewer_denied_for_owner_route_403(wiring):
    """viewer 는 OWNER 요구 라우트에서 위계 미달 → 403 (Req 4.3, INV-2)."""
    with TestClient(wiring.app) as client:

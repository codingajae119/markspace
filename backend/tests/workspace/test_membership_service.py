...
- change_role: role 갱신. 멤버십 미존재→404. 단독 owner 를 member 로 강등→409(영속 없음),
  다른 owner 가 남아 있으면 강등 허용.
- remove_member: 물리 삭제. 멤버십 미존재→404. 단독 owner 제거→409(제거 없음),
  다른 owner 가 남아 있으면 제거 허용.
...
class _FakeMemberRepo:
    ...
    def __init__(
        ...
        *,
        ...
        self.count_owners_calls: list[int] = []
    ...
    def count_owners(self, db, workspace_id: int) -> int:
        assert db is DB
        self.count_owners_calls.append(workspace_id)
...
def test_change_role_demoting_sole_owner_raises_409_and_does_not_set():
    # 단독 owner 를 member 로 강등하려는 요청은 409 로 거부되고 아무것도 영속하지 않는다.
    only_owner = _make_member(workspace_id=1, user_id=2, role="owner")
    member_repo = _FakeMemberRepo(member=only_owner, owner_count=1)
    service = MembershipService(member_repo, _FakeWsRepo())
    ...
    assert ei.value.code == ErrorCode.CONFLICT
    assert ei.value.http_status == 409
    assert member_repo.count_owners_calls == [1]
    assert member_repo.set_role_calls == []


def test_change_role_demoting_owner_with_another_owner_is_allowed():
    # owner 가 둘 이상이면 그중 하나를 member 로 강등하는 것은 허용된다(단독 owner 아님).
    an_owner = _make_member(workspace_id=1, user_id=2, role="owner")
    member_repo = _FakeMemberRepo(member=an_owner, owner_count=2)
    service = MembershipService(member_repo, _FakeWsRepo())
...
def test_change_role_reaffirming_sole_owner_as_owner_is_allowed():
    # 단독 owner 를 owner 로 재설정(무변화)하는 요청은 강등이 아니므로 owner 수와 무관하게 허용된다.
    only_owner = _make_member(workspace_id=1, user_id=2, role="owner")
    member_repo = _FakeMemberRepo(member=only_owner, owner_count=1)
    service = MembershipService(member_repo, _FakeWsRepo())

    result = service.change_role(DB, 1, 2, MemberUpdate(role=MemberRole.OWNER))

    assert member_repo.set_role_calls[0][1] == "owner"
...
def test_remove_member_removing_sole_owner_raises_409_and_does_not_remove():
    # 단독 owner 를 제거하려는 요청은 409 로 거부되고 아무것도 제거하지 않는다.
    only_owner = _make_member(workspace_id=1, user_id=2, role="owner")
    member_repo = _FakeMemberRepo(member=only_owner, owner_count=1)
    service = MembershipService(member_repo, _FakeWsRepo())
...
def test_remove_member_removing_owner_with_another_owner_is_allowed():
    # owner 가 둘 이상이면 그중 하나를 제거하는 것은 허용된다(단독 owner 아님).
    an_owner = _make_member(workspace_id=1, user_id=2, role="owner")
    member_repo = _FakeMemberRepo(member=an_owner, owner_count=2)
    service = MembershipService(member_repo, _FakeWsRepo())
    ...
    assert member_repo.remove_calls == [an_owner]


def test_remove_member_removing_non_owner_skips_owner_guard():
    # 대상이 member 이면 owner 수와 무관하게 제거되며, 단독 owner 가드는 적용되지 않는다.
    member = _make_member(workspace_id=1, user_id=2, role="member")
    member_repo = _FakeMemberRepo(member=member, owner_count=1)
    service = MembershipService(member_repo, _FakeWsRepo())

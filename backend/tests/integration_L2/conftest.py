...
- ``ws_scenario`` : admin 으로 owner/editor/viewer/비멤버 사용자를 각각 생성하고 각자
  세션을 유지하는 role별 클라이언트를 만든 뒤, owner 가 워크스페이스를 생성하고 editor·
  viewer 를 지정 role 로 멤버 추가한 **구성된 시나리오**를 반환한다. 후속 스위트(2.2~2.6)가
  role별 실제 세션으로 게이트 통과·거부·admin bypass 를 관찰하는 공용 셋업이다.
...
class WorkspaceScenario:
    ...
    - ``owner_client`` / ``editor_client`` / ``viewer_client``: 각 role 멤버의 인증 클라이언트.
...
def ws_scenario(harness) -> WorkspaceScenario:
    ...
    4. owner 가 ``POST /workspaces/{id}/members`` 로 editor·viewer 를 지정 role 로 추가(201).

    비멤버는 생성·로그인만 하고 멤버로 추가하지 않는다. admin 은 이 워크스페이스의 멤버가
    아니다(admin bypass 관찰용). 반환된 :class:`WorkspaceScenario` 로 후속 스위트가 role별
    게이트를 관찰한다.
    """
    ...
    # owner 가 editor·viewer 를 지정 role 로 멤버 추가한다(owner 게이트 통과).
    for user_id, role in ((editor_user_id, "editor"), (viewer_user_id, "viewer")):

...
3. **문서 보유 워크스페이스 삭제 거부(409) + 빈 워크스페이스 삭제 성공(204)**(7.5·7.6,
   ...
   ⚠️ **회귀 지목(s05)**: 7.5 는 409 거부 시 **멤버십**도 물리 보존을 요구하나, 현재 s05
   `WorkspaceService.delete_workspace` 는 멤버십 선삭제(`remove_all_for_workspace`)를 **독립
   커밋**한 뒤 워크스페이스 물리 DELETE 가 FK RESTRICT 로 실패하면 rollback 으로도 되돌리지
   못해 **멤버십이 제거**된다. :func:`test_workspace_delete_rejection_preserves_memberships`
   가 이를 실패로 지목한다(Requirement 1.5 — 체크포인트는 우회하지 않고 s05 수정을 유발).
...
def test_workspace_with_document_delete_rejected_409_preserves_ws_and_document(
    ...
    (:func:`test_workspace_delete_rejection_preserves_memberships`)에서 단언한다 — 현재 s05 구현이
    이를 위반하므로 그 회귀를 독립적으로 지목하기 위함이다(모듈 CONCERN 참조).
    """
...
def test_trashed_document_still_blocks_workspace_delete(ws_scenario, harness):
    ...
    (문서 삭제를 ws 삭제 **전에** 수행하는 이유: 아래 CONCERN 의 s05 멤버십 원자성 회귀로 인해
    실패한 ws 삭제 이후에는 owner 멤버십이 제거되어 후속 owner 문서 작업이 403 이 되기 때문.
    이 테스트는 그 회귀와 독립적으로 nuance 를 관찰하도록 순서를 잡는다.)
    """
    owner = ws_scenario.owner_client
...
def test_workspace_delete_rejection_preserves_memberships(ws_scenario, harness):
    """[회귀 지목] 409 로 거부된 워크스페이스 삭제는 멤버십을 물리 보존해야 한다(7.5) — 현재 s05 가 위반.

    요구사항 7.5 는 문서 보유 워크스페이스의 삭제가 409 로 거부될 때 "그 워크스페이스·문서·**멤버십**이
    물리적으로 보존됨" 을 요구하며, s05 워크스페이스 서비스의 문서화된 사후조건도 "아무것도 제거되지
    않는다" 이다. 그러나 현재 s05 구현은 이를 **위반**한다:

        WorkspaceService.delete_workspace 는
          1. MembershipRepository.remove_all_for_workspace(db, ws_id)  # 내부에서 db.commit() (repository.py:219)
          2. WorkspaceRepository.delete(db, ws)                         # FK RESTRICT → IntegrityError (repository.py:136)
        순으로 호출하고, 2 의 실패를 except 에서 db.rollback() 하지만, **1 의 멤버십 삭제는 이미
        커밋되어 있어 rollback 으로 되돌릴 수 없다**. 결과: 409 거부에도 워크스페이스·문서는 보존되나
        **멤버십은 물리적으로 제거**된다(요청자 owner 포함 → 후속 owner 작업 403 파생).

    이 회귀는 s05 가 s08 로 이연한 409 경로가 L3 에서 **처음 end-to-end 로 관측**되며 드러난다
    (design §Error Categories — s07 문서 도메인 ↔ s05 워크스페이스 삭제 경계). 체크포인트는 이를
    **실패로 보고**하되 feature 로직으로 우회하지 않는다(Requirement 1.5). 수정은 원인 spec(s05)의
    몫이다: `remove_all_for_workspace`/`delete` 의 독립 commit 을 제거하고 서비스가 단일 트랜잭션을
    소유(단일 commit)하도록 하거나, ws 물리 DELETE 를 멤버십 선삭제보다 먼저 시도해 FK RESTRICT 를
    조기 확정한 뒤 아무것도 커밋하지 않도록 한다. s05 수정 후 이 테스트는 통과해야 한다.
    """
    owner = ws_scenario.owner_client
    ...
    # 현재 s05 는 멤버십 선삭제를 독립 커밋하므로 이 단언은 실패한다(위 docstring 회귀 지목).
    assert owner_uid in _membership_user_ids(harness, ws_id), (
        f"409 로 거부된 워크스페이스 삭제는 멤버십을 물리 보존해야 한다(7.5, 원자성): "
        f"owner={owner_uid} 관측 멤버십={_membership_user_ids(harness, ws_id)} "
        f"— 현재 s05 원자성 회귀(remove_all_for_workspace 독립 commit). 수정은 s05 소유."
    )

...
# =============================================================================
# 그룹 2: 읽기 전역 개방 (조회: 상세·목록) — s26 Req 3.1·3.2·3.8·7.2
#   매트릭스: owner 200 · member 200 · 비멤버 활성 사용자 200 (403 아님 — 헤드라인 전환)
# =============================================================================


def test_get_detail_read_is_globally_open(ws_scenario):
    """상세 조회 전역 개방(`GET /documents/{id}`): owner·member·비멤버 활성 사용자 모두 200.

    member 가 만든 문서를 각 세션으로 조회한다. s26 읽기 개방(Req 3.1·3.8·7.2)으로 owner·member
    는 물론 비멤버 활성 사용자(viewer·nonmember)도 200 이다(더 이상 403 아님 — 존재 문서를 읽기에
    멤버십을 요구하지 않는다). 어댑터 매핑은 성공하고 읽기 게이트가 열려 있다.
    """
    ws_id = ws_scenario.workspace_id
    ...
    for label, client in (
        ...
        assert resp.status_code == 200, (
            f"{label} 는 상세 조회 전역 개방으로 200 이어야 한다(403 아님, 3.8): "
            f"{resp.status_code} {resp.text}"
        )


def test_list_read_is_globally_open(ws_scenario):
    """목록 조회 전역 개방(`GET /workspaces/{id}/documents`): owner·member·비멤버 모두 200.

    ``GET /workspaces/{id}/documents`` 는 경로 workspace_id 로 게이트되지만 s26 읽기 개방으로
    활성 사용자면 멤버십과 무관하게 200 ``Page[DocumentRead]`` 이다(비멤버도 403 아님, 3.2·3.8).
    """
    ws_id = ws_scenario.workspace_id
    ...
    for label, client in (
        ...
        assert resp.status_code == 200, (
            f"{label} 는 목록 조회 전역 개방으로 200 이어야 한다(403 아님, 3.8): "
            f"{resp.status_code} {resp.text}"
        )
...
def test_adapter_existing_document_insufficient_role_maps_to_403(ws_scenario):
    """`/documents/{id}` 계열은 **존재하는** 문서 + 편집 권한 미충족을 403 으로 낸다(어댑터 위임, 3.6).

    미존재 404 와 대비되는 축이다: 문서가 존재하면 어댑터가 workspace_id 매핑에 성공하고 resolver 가
    role 을 판정하므로, 비멤버의 **편집**(member 미충족)은 404 가 아니라 403 이다. s26 읽기 개방으로
    조회는 열려 있으므로(비멤버 조회 200) 권한-미충족-403 축은 편집 계열로 관측한다 — 두 비멤버
    세션의 편집 시도 모두 403 임을 확인해 어댑터가 판정을 재구현하지 않고 resolver 에 위임함을
    미존재-404 와 함께 확정한다. 아울러 존재 문서의 비멤버 **조회**는 403 이 아니라 200(읽기 개방)임을
    대조로 확인한다.
    """
    ws_id = ws_scenario.workspace_id
    ...
    # 존재 문서 + 비멤버 편집(member 미충족) → 403 (404 아님). 두 비멤버 세션으로 관측.
    for label, client in (
        ("viewer(비멤버)", ws_scenario.viewer_client),
        ("nonmember", ws_scenario.nonmember_client),
    ):
        edit_denied = helpers.attempt_patch_title(
            client, doc_id, _title(f"{label}변경")
        )
        assert edit_denied.status_code == 403, (
            f"존재 문서 + {label} 편집(member 미충족)은 403 이어야 한다(어댑터 위임, 3.6): "
            f"{edit_denied.status_code} {edit_denied.text}"
        )

    # 대조: 존재 문서 + 비멤버 조회는 403 이 아니라 200(읽기 전역 개방, 3.8).
    nonmember_read = helpers.attempt_get_document(
        ws_scenario.nonmember_client, doc_id
    )
    assert nonmember_read.status_code == 200, (
        f"존재 문서 + 비멤버 조회는 읽기 개방으로 200 이어야 한다(403 아님, 3.8): "

"""s12 첨부 통합 테스트 하네스 (Task 4.x / design §Testing Strategy Integration Tests).

mock 없이 s01 ⊕ s02 ⊕ s03 ⊕ s05 ⊕ s07 ⊕ s09 ⊕ s10 ⊕ **s12** 의 실제 구현을 결합한
검증 환경을 제공한다. 핵심 원칙은 **L3 하네스 재사용**(중복 신설 금지)이다: 마이그레이션
(`alembic upgrade head`)·`s01` `create_app()` 부팅(첨부 라우터 + 아카이브 스케줄러 포함)·
admin 시드·세션 유지 `TestClient` 팩토리·고유 login_id 생성기·워크스페이스 생성·멤버 추가
(role)·role별 세션 클라이언트·문서 트리 생성·부팅 앱과 동일 `SessionLocal`/`get_db` 세션은
모두 `s08` `tests/integration_L3` 자산(그리고 그것이 재사용하는 `s06` `tests/integration_L2`·
`s04` `tests/integration_L1`)을 그대로 쓴다. 부팅 앱은 s02·s03·s05·s07·s09·s10·**s12 첨부
라우터가 조립된 상태**(`app.main.create_app`)이므로 첨부 업로드·조회 서빙 라우트(s01 카탈로그
행 32~33)가 노출된다.

pytest 는 fixture 를 정의된 디렉터리 트리에서만 수집하므로, 형제 디렉터리(L3/L2/L1)의
fixture 는 `tests/attachment/` 에서 보이도록 **명시적 re-import** 해야 한다(L4 conftest 의
re-import 패턴 답습). L3 conftest 의 ``__all__`` 이 이미 `harness`(L1)·`ws_scenario`/
`WorkspaceScenario`(L2)·문서 트리·엔진 접근을 자신의 네임스페이스로 재-export 하므로, 이
모듈은 그 하나의 지점에서 하네스·워크스페이스 시나리오·문서 트리·엔진 접근 픽스처를 한꺼번에
재-import 한다.

이 모듈이 신규로 추가하는 것은 첨부 통합 테스트 전용 두 픽스처다:
...
1. ``tmp_attachment_roots`` (:class:`AttachmentStorageRoots`): 저장/보관 루트를 tmp_path
   ...
   경로(라우터→서비스→스토리지)와 아카이브 스윕(참조 소멸·완전삭제 반응) 양쪽을 모두 tmp
   루트로 돌려 실제 `./var/...` 저장 루트를 오염시키지 않는다(test_archival.py 의 ``roots``
   픽스처와 동일 규약). 노출된 ``file_storage_root``/``attachment_archive_root`` 경로로
   테스트가 디스크상 WS 격리 저장을 직접 단언한다.
2. ``archival_sweep`` (:class:`ArchivalSweepAccess`): 부팅 앱과 **동일 세션 팩토리**
   ...
   ``sweep`` 을 1회 구동하고 처리 건수를 반환한다(L4 `SweepAccess` 미러). 4.2/4.3 아카이브
   seam 통합 테스트가 이 하나의 접근 핸들로 완전삭제 반응·참조 소멸 스윕을 관찰하도록 지금
   함께 둔다.

제약(design §Testing Strategy):
- 어떤 애플리케이션 코드·`config.yml`·L3/L2/L1 하네스 자산도 수정하지 않는다(재사용만).
- mock·stub 미사용(스윕 직접 호출은 실제 s12 코드 실행이므로 허용). 설정은 s01 `Settings`
  재사용(첨부 additive 필드 포함, L1 하네스 경유)이며, storage 루트만 tmp 로 격리한다.
...
class AttachmentStorageRoots:
    ...
    - ``attachment_archive_root``: 아카이브 스윕이 파일을 옮기는 tmp 보관 루트(4.2/4.3용).
    """
...
def tmp_attachment_roots(tmp_path, monkeypatch) -> AttachmentStorageRoots:
    ...
    API 업로드(라우터→서비스→스토리지)와 아카이브 스윕이 모두 tmp 하위에 쓰기·이동하게 되어
...
class ArchivalSweepAccess:
    ...
    mock 하지 않는다. 4.2/4.3 seam 통합 테스트가 이 핸들로 스윕을 구동한다.
    ...
    def sweep(self, now: datetime) -> int:
        ...
        관찰한다. 세션 수명(commit·close)은 이 핸들이 소유한다.
        """
...
def archival_sweep(harness) -> ArchivalSweepAccess:
    ...
    정렬). ``now`` 는 :meth:`ArchivalSweepAccess.sweep` 호출 시 주입해 결정성을 확보한다.
    """

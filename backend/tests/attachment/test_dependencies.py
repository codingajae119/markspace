"""AttWsAdapter 단위/통합 검증 (s12-attachment task 1.5 / Req 3.4, 3.6, 7.3).

design.md §Components and Interfaces #AttWsAdapter 검증:
- `/attachments/{id}` 라우트는 경로 파라미터가 첨부 id 이며 s01 `require_ws_role` 의 내부
  의존성은 경로 파라미터 `workspace_id` 를 문자 그대로 읽는다. 따라서 s12 는 첨부 id →
  workspace_id 로 잇는 **얇은 어댑터**(`ws_role_for_attachment`)를 제공하고 판정(위계·admin
  bypass·403)은 전부 s01 resolver 에 위임한다(재구현 없음, 7.3).
- 첨부는 `workspace_id` 를 직접 보유하므로(문서 조회 불필요) 단건 조회로 매핑하며, 첨부
  미존재 시에는 s01 판정에 앞서 404(`DomainError(NOT_FOUND)`)로 거부한다(Req 3.6).
...
def wiring():
    ...
    suffix = uuid4().hex[:8]  # 병렬/재실행 식별자 충돌 회피.
    ...
    try:
        ...
        att = Attachment(
            workspace_id=ws.id,
            document_id=doc.id,
            file_path="ws-x/att-x.png",
            original_name="붙여넣기.png",
            kind="image",
            is_archived=False,
            created_at=datetime.utcnow(),
        )
...
def test_admin_bypasses_to_200(wiring):
    """admin 은 멤버십·role 무관하게 어댑터를 통과 → 200 (Req 7.3, INV-3 admin bypass)."""
    with TestClient(wiring.app) as client:
...
# --- 첨부 미존재 → 404 (판정 이전 매핑 실패, Req 3.6) ---
...
def test_missing_attachment_404_precedes_admin(wiring):
    """admin 이라도 첨부 자체가 없으면 판정에 앞서 404 를 낸다(매핑이 판정보다 먼저)."""
    with TestClient(wiring.app) as client:
        assert client.post(f"/_test/login/{wiring.admin_id}").status_code == 200
        resp = client.get(f"/_att_probe/{wiring.missing_attachment_id}")
...
def test_unauthenticated_401(wiring):
    """세션 없이 요청하면 s01 get_current_user 가 401 을 낸다(첨부 조회 이전)."""
    with TestClient(wiring.app) as client:
        resp = client.get(_probe_url(wiring))

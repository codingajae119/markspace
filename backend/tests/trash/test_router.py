...
- 세 경로 모두 EDITOR 게이트를 부착한다. 목록은 {id}=workspace_id 브리지,
  복구·완전삭제는 {bundleId}→WS 어댑터를 사용한다.
- 성공 계약: 목록 200 Page[TrashBundleRead], 복구 204(no body), 완전삭제 204(no body).
- 위임: 각 핸들러가 올바른 인자로 대응 서비스 메서드를 호출한다.
- 미인증 → 401, viewer/비멤버 → 403, admin → bypass, 유효하지 않은 묶음 루트 → 404.
...
def test_editor_gate_editor_passes(method, path):
    app, _ = _build_app()
...
def test_editor_gate_below_editor_forbidden_403(method, path, role):
    """viewer·비멤버 → 403(Req 1.7·2.5·3.6·5.2·INV-2)."""
    app, _ = _build_app()
    _login(app, user_id=3, is_admin=False)

...
- [x] 5.1 CRUD·계층·권한 게이팅 통합 테스트
  ...
  - _Depends: 4.2_
- [x] 5.2 (P) 상태 엔진 불변식 property·edge-case 테스트
...
- 5.2: **s01 스키마 정밀도 한계(다운스트림 주의)** — `document.trashed_at` 은 MySQL `DATETIME(0)`
  ...
  (sub-second 비흡수 불가). 엔진 로직은 정상 — 저장 해상도 한계이며 s07 이 수정할 수 없음(스키마=s01
  소유). s10 보관 타이머(trashed_at 기준)·s09 는 sub-second 삭제 순서를 구분할 수 없음을 전제할 것.
  테스트는 count/독립성 단언에 명시적 distinct-timestamp 시드(T1<T2)를 써서 이 한계를 우회한다.

...
- 조립 통합 테스트 경합 함정: 세션 우회(`is_admin`)로 노출되는 요소(예: 멤버 관리 "멤버 추가" 버튼)는 **즉시** 렌더되지만 role 신호(`roleFor`)는 시드 effect(workspaces 커밋 **후**)로 채워진다. 버튼 노출을 `waitFor` 한 뒤 role 을 **동기 읽기**하면 부하 시 시드 미완 경합으로 flake(4.2 `roleSeedAssembly.integration.test.tsx` admin 케이스에서 4.3 착수 중 포착). role 단언은 반드시 시드 의존 신호를 `waitFor` 로 기다린다.

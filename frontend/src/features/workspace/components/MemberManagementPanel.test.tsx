...
beforeEach(() => {
  ...
  // 렌더하는 테스트가 공통으로 의존하는 기본 조회 상태. 추가 폼은 배정 가능 사용자가 0명이면 숨겨지므로
  // 기본값으로 후보 1명을 두어 폼(멤버 추가 버튼)이 렌더되게 한다. 빈 상태는 각 테스트가 재설정.
  setAssignableUsers({ status: "ready", users: [assignable(101, "배정후보")] });
  setWorkspaceMembers();
});
...
describe("MemberManagementPanel — owner 게이팅(실 RequireRole)·뮤테이션 결선(Req 3.x·7.x)", () => {
  ...
  it("추가 폼에는 역할 combo 가 없고 멤버 추가 버튼만 노출한다(추가 역할은 member 고정)", () => {
    mockAuthenticatedNonAdmin();
    setWorkspace(ws(WS_ID));
    setRoleFor(Role.OWNER);
    setMemberActions();
    ...
    // 추가 폼: 사용자 선택 + 멤버 추가 버튼은 있으나 역할 combo(label "역할")는 없다.
    expect(screen.getByRole("option", { name: "사용자 선택" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "멤버 추가" })).toBeInTheDocument();
    expect(screen.queryByLabelText("역할")).not.toBeInTheDocument();
  });

  it("멤버 목록의 role 변경 combo 는 owner/member 2값만 노출한다 (Req 3.4·6.2)", () => {
    mockAuthenticatedNonAdmin();
    setWorkspace(ws(WS_ID));
    setRoleFor(Role.OWNER);
    setMemberActions();
    ...
    const roleSelect = screen.getByLabelText("7 그레이스 역할") as HTMLSelectElement;
  ...
  it("사용자 선택 후 추가 → 선택된 user_id·member 역할로 add 호출, 이후 로스터·assignable 재조회 (Req 4.1·4.3)", async () => {
    mockAuthenticatedNonAdmin();
    setWorkspace(ws(WS_ID));
    setRoleFor(Role.OWNER);
    setMemberActions();
    ...
    // 역할 combo 제거 → 추가 역할은 항상 member 고정.
  ...
  // === 단독 owner 보호(하드 불변식): 강등·제거 컨트롤 비활성화 ===

  it("owner 가 한 명뿐이면 그 owner 의 role 변경·제거 컨트롤을 비활성화하고 사유를 표기한다", () => {
    mockAuthenticatedNonAdmin();
    setWorkspace(ws(WS_ID));
    setRoleFor(Role.OWNER);
    setMemberActions();
    // owner 1명 + member 1명 → owner 는 단독. member 는 정상 조작 가능해야 한다.
    setWorkspaceMembers({
      members: [rosterRow(3, "앨리스", "owner"), rosterRow(4, "밥", "member")],
    });

    render(<MemberManagementPanel />);

    // 단독 owner(3 앨리스): role select·제거 버튼 모두 비활성 + 사유 문구 노출.
    expect(screen.getByLabelText("3 앨리스 역할")).toBeDisabled();
    expect(screen.getByRole("button", { name: "3 앨리스 제거" })).toBeDisabled();
    expect(screen.getByText("단독 owner 는 강등하거나 제거할 수 없습니다.")).toBeInTheDocument();

    // 일반 member(4 밥): 정상적으로 조작 가능(가드는 단독 owner 에만 적용).
    expect(screen.getByLabelText("4 밥 역할")).not.toBeDisabled();
    expect(screen.getByRole("button", { name: "4 밥 제거" })).not.toBeDisabled();
  });

  it("owner 가 둘 이상이면 각 owner 를 강등·제거할 수 있다(단독 아님 → 가드 미적용)", () => {
    mockAuthenticatedNonAdmin();
    setWorkspace(ws(WS_ID));
    setRoleFor(Role.OWNER);
    setMemberActions();
    setWorkspaceMembers({
      members: [rosterRow(3, "앨리스", "owner"), rosterRow(5, "이브", "owner")],
    });

    render(<MemberManagementPanel />);

    // owner 가 둘이므로 어느 쪽도 단독이 아니다 → 두 owner 모두 조작 가능, 사유 문구 없음.
    expect(screen.getByLabelText("3 앨리스 역할")).not.toBeDisabled();
    expect(screen.getByRole("button", { name: "3 앨리스 제거" })).not.toBeDisabled();
    expect(screen.getByLabelText("5 이브 역할")).not.toBeDisabled();
    expect(screen.getByRole("button", { name: "5 이브 제거" })).not.toBeDisabled();
    expect(screen.queryByText("단독 owner 는 강등하거나 제거할 수 없습니다.")).not.toBeInTheDocument();
  });
  ...
  it("추가 성공 후 assignable.reload 로 목록이 비면 추가 폼 전체가 사라진다 (Req 3.1·3.4)", async () => {
    mockAuthenticatedNonAdmin();
    setWorkspace(ws(WS_ID));
    setRoleFor(Role.OWNER);
    setMemberActions();
    ...
    // reload 로 목록이 비면 배정 가능 0명 → 추가 폼(선택지·멤버 추가 버튼)이 통째로 사라진다.
    expect(screen.queryByRole("option", { name: /그레이스/ })).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "멤버 추가" })).not.toBeInTheDocument();
  });

  it("배정 가능 0명이면 추가 폼(멤버 추가 버튼·안내 문구)을 통째로 숨긴다 (Req 3.5)", () => {
    mockAuthenticatedNonAdmin();
    setWorkspace(ws(WS_ID));
    setRoleFor(Role.OWNER);
    setMemberActions();
    ...
    // 추가 폼 전체 은닉: 멤버 추가 버튼·사용자 선택·EmptyState 안내 모두 렌더되지 않는다.
    expect(screen.queryByRole("button", { name: "멤버 추가" })).not.toBeInTheDocument();
    expect(screen.queryByRole("option", { name: "사용자 선택" })).not.toBeInTheDocument();
    expect(screen.queryByText("배정 가능한 사용자가 없습니다")).not.toBeInTheDocument();
    // 패널 자체(멤버 관리 헤더)는 그대로 노출된다.

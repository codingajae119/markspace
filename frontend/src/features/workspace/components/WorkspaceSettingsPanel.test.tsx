...
describe("WorkspaceSettingsPanel — owner 게이팅(실 RequireRole)·설정 뮤테이션 결선(Req 4.x·7.x)", () => {
  ...
  it("현재 WS 가 바뀌면 이름 입력창이 새 WS 이름으로 갱신된다 (재로그인 last_selected 재지정 회귀, key 재마운트)", () => {
    mockAuthenticatedNonAdmin();
    // 최초: 워크스페이스 목록만 확정된 시점(무관한 WS 가 currentWorkspace).
    setWorkspace(ws({ id: 7, name: "무관한워크스페이스" }));
    setRoleFor(Role.OWNER);
    setActions();

    const { rerender } = render(<WorkspaceSettingsPanel />);
    expect(screen.getByLabelText("워크스페이스 이름")).toHaveValue("무관한워크스페이스");

    // last_selected_workspace_id 비동기 재지정 → currentWorkspace 가 실제 선택 WS 로 교체.
    setWorkspace(ws({ id: 42, name: "현재워크스페이스" }));
    rerender(<WorkspaceSettingsPanel />);

    expect(screen.getByLabelText("워크스페이스 이름")).toHaveValue("현재워크스페이스");
  });

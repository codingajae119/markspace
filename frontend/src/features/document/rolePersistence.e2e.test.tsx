/**
 * s24-role-persistence — provider-role 파급 복원 회귀 + 4-role E2E critical paths (task 4.3).
 *
 ...
 *
 * Requirements: 2.2(provider-role 비-null 복원), 4.3(editor/viewer 차단), 4.4(admin 세션 경로·role
 * 미접합), 5.4(role 신호에 admin override 미접합).
 * Design: §CurrentWorkspaceProvider Risks(파급 복원), §Testing Strategy E2E/UI(4 role 시나리오).
 */
...
/** 4-role 시나리오 공통 씬: 배지 + 멤버 관리 패널 + 문서 툴바 + 잠금 배너 + role 신호 프로브. */
...
//    provider-role 이 null→실값으로 바뀌며 소비처 게이팅이 editor/owner 에게 의도대로 노출됨.
// ===========================================================================================

describe("파급 복원 회귀 — provider-role null→실값으로 소비처 게이팅 노출 (Req 2.2)", () => {
  it("editor: 복원된 provider-role 로 DocumentToolbar 조작이 노출된다(과거 null 이면 은닉)", async () => {
    // 비-admin editor 세션. role 은 오직 mocked /workspaces 응답에서만 온다(주입 없음).
    installAssemblyFetch({ isAdmin: false, role: "editor" });

    renderAssembly(<DocumentToolbarProbe />);

    // 새 문서 생성 컨트롤이 노출된다. provider-role 이 null 하드코딩이던 과거엔 RequireRole(EDITOR)
  ...
  it("owner: DocumentToolbar + EditLockBanner 강제 해제(OWNER 게이트)가 함께 노출된다", async () => {
    ...
    // editor+ 게이트 통과(툴바) + OWNER 게이트 통과(강제 해제) 모두 복원된 provider-role 로 열린다.
    await waitFor(() => expect(createButton()).toBeInTheDocument());
    expect(forceUnlockButton()).toBeInTheDocument();
  });

  it("viewer: 복원된 provider-role 가 VIEWER 라 editor+ 게이트가 여전히 닫혀 있다(파급이 과노출 아님)", async () => {
    installAssemblyFetch({ isAdmin: false, role: "viewer" });
    ...
    // 시드 완료의 양성 신호(배지=viewer) 확인 후 음성 단언 — provider-role 이 VIEWER 로 복원되었음에도
    // editor+ 게이트가 닫혀 있음을 증명(파급 복원이 무차별 노출이 아님).
    await roleBadge("viewer");
...
// ===========================================================================================
// B. 4-role E2E critical paths (각 = 새 마운트, in-session 이력 없음 = 새로고침)
//    design §Testing Strategy E2E/UI Tests.
// ===========================================================================================

describe("4-role E2E — 새로고침 후 role 복원 critical paths (design §E2E/UI)", () => {
  it("owner 새로고침: 배지=owner + 멤버 관리 접근 + 문서 툴바 노출", async () => {
    ...
    // owner 멤버 관리 패널 접근 복원(Req 4.1) + editor+ 문서 툴바 노출.
  ...
  it("editor 새로고침: 문서 툴바 노출 + 멤버 관리 은닉 + 배지=editor", async () => {
    installAssemblyFetch({ isAdmin: false, role: "editor" });

    renderAssembly(<Scene />);

    // 시드 완료 양성 신호(배지=editor) 후 게이팅 단언.
    await roleBadge("editor");
    expect(createButton()).toBeInTheDocument(); // 문서 툴바 노출
    expect(memberPanel()).toBeNull(); // 멤버 관리 은닉(editor < OWNER, Req 4.3)
    expect(forceUnlockButton()).toBeNull(); // OWNER 강제 해제도 은닉
  });

  it("viewer 새로고침: 읽기 전용(툴바·멤버 관리 미노출) + 배지=viewer", async () => {
    installAssemblyFetch({ isAdmin: false, role: "viewer" });

    renderAssembly(<Scene />);

    await roleBadge("viewer");
    expect(createButton()).toBeNull(); // 문서 툴바 미노출(viewer < EDITOR)
    expect(memberPanel()).toBeNull(); // 멤버 관리 미노출(viewer < OWNER, Req 4.3)
    expect(forceUnlockButton()).toBeNull();
  });

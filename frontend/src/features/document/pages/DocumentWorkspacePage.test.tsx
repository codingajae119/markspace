...
describe("DocumentWorkspacePage 조립 (트리 + 브레드크럼 + 뷰어 + 툴바)", () => {
  ...
  it("editor role 이면 툴바(생성 컨트롤)를 노출한다 (Req 9.6)", async () => {
    ...
    // 설정 판넬은 기본 숨김이므로, editor 는 토글 버튼으로 열 수 있어야 한다.
    const showToggle = await screen.findByRole("button", { name: "문서 설정 보기" });
  ...
  it("editor+ 가 뷰어의 삭제 버튼을 눌러 확인하면 deleteDocument(id) 로 휴지통 이동을 배선한다 (Req 5.1)", async () => {
    setReadyWorkspace();
    setAuthenticated();
    loadAllActiveDocumentsMock.mockResolvedValue([sampleDoc()]);
    getDocumentMock.mockResolvedValue(sampleDoc());
    ...
    fireEvent.click(await screen.findByText("설계 노트"));
    // 뷰어 상세 로드 완료 후 편집 버튼 옆에 삭제 버튼이 노출된다(canEdit=true, onDelete 배선).
  ...
  it("삭제 실패 시 변이 오류를 설정 판넬 개폐와 무관하게 페이지 레벨로 표면화한다 (Req 5.1, 9.4)", async () => {
    setReadyWorkspace();
    setAuthenticated();
    loadAllActiveDocumentsMock.mockResolvedValue([sampleDoc()]);
    getDocumentMock.mockResolvedValue(sampleDoc());
    ...
    fireEvent.click(await screen.findByText("설계 노트"));
    fireEvent.click(await screen.findByRole("button", { name: "삭제" }));
    // 설정 판넬은 기본 숨김 상태 그대로 — 그래도 삭제 오류가 페이지에 노출되어야 한다.
    fireEvent.click(screen.getByRole("button", { name: "휴지통으로 이동" }));
  ...
  it("문서가 없는 워크스페이스는 빈 상태 안내를 표시하되 생성 툴바는 유지한다 (Req 1.6)", async () => {
    ...
    // 설정 판넬은 기본 숨김이므로 토글로 열어 생성 컨트롤을 확인한다.
    fireEvent.click(screen.getByRole("button", { name: "문서 설정 보기" }));
  ...
  it("문서 설정 판넬 토글로 상단 생성·이름변경 툴바를 숨기고 다시 표시할 수 있다", async () => {
    ...
    // 기본은 설정 컨트롤이 숨겨져 있고, 토글 버튼은 "보기" + aria-expanded=false.
    // (트리는 로드되므로 트리 노드로 렌더 완료를 기다린다.)
    expect(await screen.findByText("설계 노트")).toBeInTheDocument();
    expect(screen.queryByLabelText("새 문서 제목")).not.toBeInTheDocument();
    const showToggle = screen.getByRole("button", { name: "문서 설정 보기" });
    expect(showToggle).toHaveAttribute("aria-expanded", "false");

    // 보기 → 판넬이 렌더에 포함되어 생성·이름변경 컨트롤이 나타나고 라벨이 뒤집힌다.
    fireEvent.click(showToggle);
    expect(await screen.findByLabelText("새 문서 제목")).toBeInTheDocument();
    const hideToggle = screen.getByRole("button", { name: "문서 설정 숨기기" });
    expect(hideToggle).toHaveAttribute("aria-expanded", "true");

    // 다시 숨기기 → 설정 컨트롤이 렌더에서 제외된다.
    fireEvent.click(hideToggle);
    expect(screen.queryByLabelText("새 문서 제목")).not.toBeInTheDocument();
    expect(screen.queryByLabelText("문서 이름 변경")).not.toBeInTheDocument();
    expect(screen.getByRole("button", { name: "문서 설정 보기" })).toBeInTheDocument();
  });

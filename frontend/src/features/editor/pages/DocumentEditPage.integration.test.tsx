...
describe("DocumentEditPage 종단 통합 — 붙여넣기/드롭 → 업로드 → 자리표시자 치환 (조립 갭 가드)", () => {
  ...
  it("렌더러 결선(배선 검증): buildAttachmentRenderers() 산출물이 EditorWrapper renderers 에 도달한다 (R2.1·2.2)", () => {
    ...
    // NOTE(D3): 라이브 blob DOM 실렌더 검증은 s16 소유 `.outerHTML` 직렬화 seam 한계로 유보한다
    // (React 19 createRoot 비동기 커밋 vs 동기 outerHTML read). 이 테스트는 렌더러가 편집 표면
    // slot 까지 "결선"됨(배선)만 단언하며, s16 seam 이 라이브 마운트로 근본 수정되면 검증 깊이를
    // 실 blob 렌더까지 상향한다(design.md 재검증 트리거).
  });
